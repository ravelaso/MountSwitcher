...
local function RebuildMountDatabase()
    ...
    -- C_MountJournal.GetMountIDs() returns every mount the account owns.
    -- We filter to collected+usable mounts only.
    C_MountJournal.SetCollectedFilterSetting(LE_MOUNT_JOURNAL_FILTER_COLLECTED, true)
    C_MountJournal.SetCollectedFilterSetting(LE_MOUNT_JOURNAL_FILTER_NOT_COLLECTED, false)

    local mountIDs = C_MountJournal.GetMountIDs()
    for _, mountID in ipairs(mountIDs) do
        -- GetMountInfoByID: name, spellID, icon, active, isUsable, sourceType,
        --   isFavorite, isFactionSpecific, faction, hideOnChar, isCollected, mountID
        local name, spellID, icon, _, isUsable, _, _, _, _, _, isCollected =
            C_MountJournal.GetMountInfoByID(mountID)

        if spellID and isCollected and isUsable then
            -- GetMountInfoExtraByID: description, source, sourceText,
            --   isSelfMount, mountTypeID, ...
            local _, _, _, _, mountTypeID = C_MountJournal.GetMountInfoExtraByID(mountID)

            -- mountTypeID 247 = ground+fly capable, 248 = flying, 424 = AQ flying
            -- 230 = ground only, 269 = aquatic, etc.
            local flyingTypes = { [247] = true, [248] = true, [424] = true }
            local isFlying = flyingTypes[mountTypeID] or false
...
local function UpdateSecureButton()
    ...
    local spellName = C_Spell.GetSpellName(mountData.spellID)
    if not spellName then
        Debug("C_Spell.GetSpellName nil for spellID", mountData.spellID)
        return
    end
